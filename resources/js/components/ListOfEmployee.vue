    ...
    <table class="table table-bordered">
      ...
      <tbody>
        ...
        >
          ...
          <td>{{ (employee.user && employee.user.status === 'active') ? 'Active' : 'Inactive' }}</td>
        </tr>
      </tbody>
    </table>
  </div>
...
export default {
  setup() {
    ...
    const fetchEmployees = (page = 1) => {
      // Ensure the page value is within bounds
      if (page < 1 || page > totalPages.value) return;
      
      axios
        .get(`/api/admin/employees?page=${page}`)
        .then(response => {
          const { data, current_page, last_page } = response.data; // Destructure pagination data
    ...
    const filteredEmployees = computed(() => {
      ...
      return employees.value.filter(employee => {
        return (
          ...
          ((employee.user?.status === 'active') ? 'active' : 'inactive').includes(query)
        );
      });
    });
